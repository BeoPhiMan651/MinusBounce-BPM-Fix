...
import net.minusmc.minusbounce.utils.block.BlockUtils.searchBlocks
...
@ModuleInfo(name = "Nuker", spacedName = "Nuker", description = "NUKE ALL", category = ModuleCategory.WORLD)
class Nuker : Module() {
    ...
    fun onUpdate(event: UpdateEvent) {
        if (blockHitDelay > 0 && !MinusBounce.moduleManager[SpeedMine::class.java]!!.state) {
            blockHitDelay--
            return
        }
        ...
        if (!mc.playerController.isInCreativeMode) {
            ...
            do{
                ...
                // Call auto tool
                val autoTool = MinusBounce.moduleManager.getModule(AutoTool::class.java) as AutoTool
                if (autoTool.state)
